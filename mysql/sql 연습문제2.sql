...
insert into `bank_customer` values ('102-22-50194', '부산의원', 2, '051-518-1010', '부산시 남구');
...
select
    ...
    count(case when `t_dist` = 1 then `t_dist` end) as '출금 건수',
    count(case when `t_dist` = 1 then `t_dist` end) as '조회 건수'
from `bank_transaction`;
